...
<body>
  ...
  <div id="quick-view-modal" class="quick-view-modal is-visible" role="dialog" aria-modal="true" style="z-index: calc(var(--z-index-overlay, 1999) + 10);">
    ...
    return slug ? `<a href="${baseUrl}?_p=/${slug}/">${escapeHtml(preserveHyphenatedWords(name))}</a>${i < rawGenres.length - 1 ? ", " : ""}` : `<span>${escapeHtml(preserveHyphenatedWords(name))}</span>${i < rawGenres.length - 1 ? ", " : ""}`;
  }).join("")}
                  </span>
                </div>
              ` : ""}
...
<body class="collection-wall">
  ...
  <script>
    (function () {
      ...
      // Sincronizaci\xF3n e interactividad del bot\xF3n de tema claro/oscuro
      var themeBtn = document.getElementById("theme-toggle");
      if (themeBtn) {
        var updateThemeBtn = function (isDark) {
          themeBtn.setAttribute("aria-pressed", String(isDark));
      ...
      <div class="flip-card-inner">
        ...
        <div class="flip-card-front">
          <div class="poster-container">
            ...
            ${hasBothRoles ? `
              <div class="person-role-controls" role="group" aria-label="Cambiar filmograf\xEDa">
                <button
                  type="button"
                  class="person-role-btn ${currentRole === "director" ? "is-active" : ""}"
                  data-role="director"
                  title="Ver pel\xEDculas de ${escapeAttr(person.name)} como Director"
                  aria-label="Ver pel\xEDculas de ${escapeAttr(person.name)} como Director"
                ...
                <button
                  ...
                  data-role="actor"
                  title="Ver pel\xEDculas de ${escapeAttr(person.name)} como Actor"
                  aria-label="Ver pel\xEDculas de ${escapeAttr(person.name)} como Actor"
