...
<body>
  ...
  <div id="quick-view-modal" class="quick-view-modal is-visible" role="dialog" aria-modal="true">
    ...
    return `
                ...
                <div class="year-country-line">
                  <div class="year-flag-group">
                    ...
                    ` : ""}
                    ${countryCode ? `
                      <a href="${countrySlug ? `${baseUrl}pais/${countrySlug}/` : "#"}" class="country-info" data-template="country-container" style="display:flex; text-decoration:none;" title="${escapeAttr(countryName)}" aria-label="Ver t\xEDtulos de ${escapeAttr(countryName)}">
    ...
    return slug ? `<a href="${baseUrl}genero/${slug}/">${escapeHtml(preserveHyphenatedWords(name))}</a>${i < rawGenres.length - 1 ? ", " : ""}` : `<span>${escapeHtml(preserveHyphenatedWords(name))}</span>${i < rawGenres.length - 1 ? ", " : ""}`;
  }).join("")}
                  </span>
                </div>
              ` : ""}
