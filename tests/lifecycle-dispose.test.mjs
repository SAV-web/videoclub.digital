...
describe("Modal SPA abierta directamente (SEO / ?movie={id})", () => {
  ...
  test("Apertura desde SEO (?movie={id}): no crea entrada pushState duplicada y closeModal ejecuta history.back() si hay historial previo", async () => {
    modalModule.disposeModalEvents();
    ...
    // Simulamos que el usuario llegó desde una página SEO (document.referrer y history.length > 1)
    let pushStateCalled = false;
    let backCalled = false;
    globalThis.window.history.pushState = () => { pushStateCalled = true; };
    globalThis.window.history.length = 2;
    globalThis.document.referrer = "https://videoclub.digital/director/clint-eastwood/";
    globalThis.window.history.state = null;
    globalThis.window.history.onBack = () => { backCalled = true; };

    const dummyMovie = {
      id: 500,
      title: "Sin Perdón",
      posterUrl: "https://ejemplo.com/unforgiven.jpg",
      year: 1992,
      genres: "Western",
      parsedDirectors: ["Clint Eastwood"],
      parsedActors: ["Clint Eastwood", "Gene Hackman"]
    };

    // 1. Abrir con openedFromUrl: true (como en la carga inicial desde SEO)
    modalModule.openModalForMovie(dummyMovie, { openedFromUrl: true });
    await new Promise((r) => setTimeout(r, 20));

    assert.equal(pushStateCalled, false, "NO debe llamar a pushState al abrir desde URL/SEO (evita duplicar el historial)");
    assert.equal(globalThis.window.history.state?.modalOpen, true, "history.state debe tener modalOpen = true");
    assert.equal(globalThis.window.history.state?.openedFromUrl, true, "history.state debe tener openedFromUrl = true");

    // 2. Cerrar modal: al haber historial previo (página SEO), debe hacer back() directamente a la página SEO
    modalModule.closeModal();
    assert.equal(backCalled, true, "closeModal debe llamar a history.back() para regresar de inmediato a la página SEO");

    modalModule.disposeModalEvents();
    globalThis.document.getElementById = origGetElementById;
    globalThis.window.history.onBack = null;
  });

  test("Apertura desde enlace directo en pestaña nueva (history.length <= 1): closeModal limpia URL sin llamar a back()", async () => {
    modalModule.disposeModalEvents();
    ...
    const dummyMovie = {
      id: 501,
      title: "Gran Torino",
      posterUrl: "https://ejemplo.com/gran-torino.jpg"
    };
    ...
    modalModule.closeModal();
    assert.equal(backCalled, false, "En pestaña nueva sin historial previo, NO debe llamar a history.back()");
    assert.equal(globalThis.window.history.state?.modalOpen, false, "history.state debe quedar en modalOpen = false");
