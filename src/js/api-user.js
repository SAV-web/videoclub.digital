...
export async function fetchUserMovieData() {
    const { data, error } = await supabase.functions.invoke('get-user-movie-data', {
        method: 'GET',
    });

    if (error) {
        console.error("Error al obtener los datos de películas del usuario:", error);
...
export async function setUserMovieDataAPI(movieId, data) {
    ...
    if (error) {
        console.error(`Error al actualizar datos para la película ${movieId}:`, error);
