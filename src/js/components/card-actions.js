...
import { addMovieToList, removeMovieFromList, isMovieInList } from '../state.js';
...
async function handleCardActionClick(event) {
    ...
    if (isActive) {
        ...
        button.setAttribute('aria-label', listType === 'favorite' ? 'Añadir a favoritos' : 'Marcar como vista');
    } else {
        addMovieToList(movieId, listType);
        button.setAttribute('aria-label', listType === 'favorite' ? 'Quitar de favoritos' : 'Desmarcar como vista');
    }
...
export function updateCardActionsUI(cardElement) {
    ...
    const favButton = cardElement.querySelector('[data-action="toggle-favorite"]');
    if (favButton) {
        // ✨ CORREGIDO: Pasa el tipo en singular.
        const isFav = isMovieInList(movieId, 'favorite');
        favButton.classList.toggle('is-active', isFav);
        favButton.setAttribute('aria-label', isFav ? 'Quitar de favoritos' : 'Añadir a favoritos');
    }

    const watchedButton = cardElement.querySelector('[data-action="toggle-watched"]');
    if (watchedButton) {
        // ✨ CORREGIDO: Pasa el tipo en singular.
        const isWatched = isMovieInList(movieId, 'watched');
        watchedButton.classList.toggle('is-active', isWatched);
        watchedButton.setAttribute('aria-label', isWatched ? 'Desmarcar como vista' : 'Marcar como vista');
    }
}
