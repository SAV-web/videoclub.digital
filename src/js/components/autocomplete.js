...
export function clearAllSidebarAutocomplete(exceptForm = null) {
  document
    .querySelectorAll(SELECTORS.SIDEBAR_AUTOCOMPLETE_RESULTS)
    .forEach((container) => {
      if (
        !exceptForm ||
        container.closest(SELECTORS.SIDEBAR_FILTER_FORM) !== exceptForm
      ) {
        const input = container.parentElement.querySelector(
          SELECTORS.SIDEBAR_FILTER_INPUT
        );
