...
export function renderPagination(
  ...
  if (currentPage > 1) {
    paginationContainer.appendChild(
      createButton(currentPage - 1, "<", false, "Ir a la página anterior")
    );
  }
  ...
  if (currentPage < totalPages) {
    paginationContainer.appendChild(
      createButton(currentPage + 1, ">", false, "Ir a la página siguiente")
    );
  }
}
...
export function prefetchNextPage(currentPage, totalMovies, activeFilters) {
  ...
  if ("requestIdleCallback" in window) {
    requestIdleCallback(async () => {
      console.log(
        `%c[PREFETCH] Inactivo, precargando página ${nextPage}...`,
        "color: #007bff"
      );
      try {
        ...
        if (error.name !== "AbortError") {
          console.warn(
            `El prefetch de la página ${nextPage} falló:`,
            error.message
          );
        }
      }
